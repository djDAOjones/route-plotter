...
- `sliderScales.{sliderToLog2Value, log2ValueToSlider, bipolarSliderToLog2Value, bipolarLog2ValueToSlider, sliderToAngle, angleToSlider, formatUIValue}` (**2026-09-26 (SPL-01):** without `angleToSlider`, deleted as uncalled per §14.1)
...
| SPL-01 | SEG-013, SEG-019, SEG-003, SEG-004 | `MotionVisibilityService.js:1506-1664` → `src/utils/sliderScales.js` | UI slider maths lives inside the visibility service; 4 of 7 importers use only this | `UIController` ~20 sites; `editorPanel:462, 471`; `persistence:827` | **Pilot (§14).** Move the 7 functions verbatim; migrate all consumers in one PR; no wrapper left behind. **2026-09-26 (SPL-01 PR): done as planned.** The six live functions moved verbatim into `src/utils/sliderScales.js`, which imports nothing, and `angleToSlider` (no callers) was deleted. The three consumers import it directly and no longer import MotionVisibilityService, which loses 160 lines; there were 23 call sites at `2fb72ff`, 20 of them in UIController (§14.1's table sums to 23; its prose says 26). `tests/sliderScales.test.js` pinned every value against the old statics first and passes unchanged against the new module, and the goldens are byte-identical. Its review found one rule the table missed, that positive readouts from 10 round up: `Math.round` there would have changed 3,165 real readouts with the whole suite green. One added row pins it. | Honest module boundary | Low | S | Characterisation table (§14) | **P1 (pilot)** |
