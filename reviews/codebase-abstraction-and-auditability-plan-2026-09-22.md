...
| DEF-40 | SEG-020 | `SceneOutlineController.js:1114-1120` (`_onClick` redraws in a microtask when a summary is clicked); `SceneOutlineController.js:277-307` (`render` records each entry's `open`, redraws, then reopens the focused entry's ancestors) | **Proposed 2026-09-25 (DEF-35 part B's browser probe); awaiting the owner's decision — not yet scheduled.** In Chromium the scene outline's entries do not open and close as they should, and by the HTML standard's order of events any browser should do the same. A click on a closed entry, or Enter or Space on it, opens it empty; a second click fills it; after that, clicking never closes it. The outline is the way to author without the canvas, so keyboard and screen-reader users meet this first. Code read: for a real click, the redraw's microtask runs before the summary's own action toggles `details.open`, so the redraw records the entry as closed and draws it without content; putting focus back on the summary then opens that empty entry, and the toggle lands on the replaced element. A click made by script runs the microtask after the toggle, which is why the jsdom tests pass. | [C] Chromium 141 probe of `main`'s outline code (`uon-open-day`, the first waypoint): a real click → open, no content; a second → open with its form; a third and a fourth → still open. Enter, or Space, on another closed entry → open, no content. An entry opened by `summary.click()` from page script → open with its form. The same on the DEF-35 part B branch. The part B review's probes: one plain redraw fires `toggle` for every open entry (two in that example), and a summary click that is cancelled toggles nothing, for a click, Enter or Space. | Let the outline, not the browser, toggle the entry: in `_onClick`, cancel a summary's click (`preventDefault`), flip the remembered state and redraw. Redrawing on the `toggle` event instead needs a guard, because every redraw fires `toggle` for each open entry and would redraw again: act only when an entry's `open` differs from the remembered state. Either way, add a test whose click toggles before its microtasks run, as a real one does | One click or key press opens or closes an entry, with its content | Low | S | — | **P2** · Proposed 2026-09-25 |
