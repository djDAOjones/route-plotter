...
| DEF-08 | SEG-012 | `RenderingService.js:2188-2195`; `exporting.js:108` | Pop/grow/pulse marker scaling applies only while `isPlaying()`, so it is **missing from video export, scrubbing and paused preview** | [C] probe: radius 10.56 (playing) vs 8 (paused) | Remove the gate (beacons are closed-form). **2026-09-26 (DEF-08 PR): removed, with one guard.** Beacons are synced every frame except while markers are set to always hide (the `beacons` render step is skipped), so the scale now applies whenever they were synced. Unguarded, the scale an earlier frame left would have shown a hidden marker when paused; while playing it already did, and no longer does. The review found two more ways a beacon keeps a stale scale, a style undone to none (the beacon stays cached) and reduced motion switched on mid-session, so the scale now applies only from a beacon synced for the frame being drawn. One golden moved, as intended: three square markers in `authored-extras`' editor frames now draw at their grow or pulse beacon's scale. | play == scrub == export | Medium (visible change) | S | TST-02 | **P1** |
