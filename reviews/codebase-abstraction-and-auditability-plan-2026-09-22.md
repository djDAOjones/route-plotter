...
| DEF-17 | SEG-014 | `AreaDrawingService.js:163-169, 191` | Finishing a polygon nulls `targetWaypoint` before emitting, so `area:changed` and `area:draw-completed` carry `null`, and the sidebar and outline don't refresh | [C] probe `[["area:changed",null],["area:draw-completed",null]]`; reproduced by Codex with the real drawing service, driven by draw-start and vertex-click events; no test completes a polygon (round-2 audit) | Capture the waypoint first; add a completion test. Also correct the "double-click closes" copy. **2026-09-26 (DEF-17 PR): done as proposed, plus a selection guard.** Drawing starts only from the sidebar's Draw Area button, which shows only once Shape reads Draw, so on the straight path the sidebar already looked right (a Shape change during the draw left it stale); the visible loss was the scene outline, which refreshed without selecting the new polygon. Drawing locks only the canvas's left click, so the selection can change before the polygon closes: the outline selects the new polygon only while its waypoint is the sole selection, and the editor refresh keeps a multi-selection's scope and mixed states. The tests complete a polygon in the booted app through the events the canvas sends, with the selection kept, moved to another waypoint, or widened to a multi-selection. | Correct UI refresh | Low | S | — | **P1** |
