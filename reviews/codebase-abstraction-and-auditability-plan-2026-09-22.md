...
| DEF-41 | SEG-014, SEG-006 | `routeTrace.js:130` (the trace copies each leg's minor waypoints into its edge's control points); `GraphEdge.js:31-33, 134-141` (`_clampPoint` holds a control point to 0–1); `FlowLayer.js:232-234` (load refuses one outside 0–1) | **Proposed 2026-09-25 (DEF-35 part A's review); awaiting the owner's decision — not yet scheduled.** Since DEF-35 a traced crowd's nodes follow their major waypoints off the image, but the minor waypoints between them become control points, and those stay on the image: the model clamps them to 0–1 and load refuses anything outside it. So where a route bends off the image through a minor waypoint, its traced crowd bends at the image's edge instead, and its dots follow that kink in the editor, video export and the player. Control points drawn by hand stay on the image by design: network editing clamps them before the model (`NetworkEditService.js:606-607`, `network.js:284`). | [C] the review's probe, rerun on the DEF-35 part A branch: a route A (0.3, 0.5) → minor (1.25, 0.15) → B (1.6, 0.5) → C (0.6, 1.45), traced. The nodes resolve to (0.3, 0.5), (1.6, 0.5) and (0.6, 1.45), but edge A→B's control point is (1, 0.15). On `main` the control point is the same and the nodes stop at the image's edge too. | Let control points take the `IMAGE_COORDINATES` range, `GraphEdge._clampPoint` and `FlowLayer`'s load check together, with a trace → save → reload test (widening the model alone would save projects that do not reopen); hand-drawn control points keep their 0–1 clamp. Or decide that a traced guide's bends stay on the image, and say so | A traced crowd follows its route wherever the route goes | Low | S | DEF-35 | **P2** · Proposed 2026-09-25 |
