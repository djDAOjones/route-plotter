...
| DEF-30 | SEG-027, SEG-007 | `perf-harness.js:58-73`; `EventBus.js:74-80` | The perf harness disables autosave *before* its refusal check, and says nothing. `EventBus.once` refires when its callback throws (no production caller). | [C] reading; [C] probe (Claude and Codex) | Reorder and restore; wrap `once` in try/finally | Tool safety; bus contract | Low | S | TST-10 pins the bus | **P3** |
| DEF-06 | SEG-004, SEG-005 | `projectReset.js:20`; `persistence.js:879, 898`; `wiringBus.js:116-120`; `undoRedo.js:387-391`; `sceneOutline.js:472-473` | Five "≥2 waypoints ?" guards skip `calculatePath`'s own reset, so **branch paths, route structure, the timeline cache and the anchor report survive Clear All** and near-empty loads. The leak is latent: nothing visible reads it today. | [C] harness; **[L]** after Clear All on `uon-open-day`: branchPaths=1 (318 points), `isLinear`=false, stale anchor report. However, **no ghost is drawn** (0 stroke calls) and the time display is correct. **2026-09-26 (DEF-08's review):** no longer latent once DEF-08 lands. Deleting a route down to one waypoint takes the same kind of guard (`wiringBus.js:119-123`), so the beacon schedules survive too, and the DEF-08 branch draws that last marker, paused, at a grow scale of 1.898 from a route that no longer exists (the review's B4). On `main` it shows only while playing. | Always call `calculatePath()` | No stale derived state for a future consumer to trip over | Low | S | TST-06 | **P3** (was P1; downgraded after runtime check) |
...
| DEF-38 | SEG-012 | `RenderingService.js:463-522` (the viewport zoom's `save`/`restore` around the background pass), and the `save`s in `renderBackgroundWithAOV`, `renderBackgroundWithSpotlight`, `renderBackgroundWithReveal`, `renderBackground` and `renderOverlay` | **Proposed 2026-09-25 (DEF-36 PR); accepted by the owner 2026-09-25.** DEF-36's exposure, on the main canvas. A throw inside one of those saves skips its `restore`, so the main canvas keeps that frame's viewport zoom or camera transform, and every later frame draws the background, and composites the route, through it until a resize. In the spotlight, angle-of-view and reveal paths the save also holds `globalCompositeOperation = 'destination-in'`, set just before a `createRadialGradient` (which throws on a negative or non-finite radius) or the mask's `drawImage`; left in force, it blanks most of the picture. DEF-36's cure does not carry over: the main canvas's base transform and smoothing belong to its host (the editor's `scale(dpr)`, set at a resize; the export canvas's identity; the player's render scale), so the renderer cannot simply resize it. No known trigger: the calls inside those saves draw a decoded background, the reveal mask and gradients from validated settings. | [C] probes on the DEF-36 branch (the test recorder keeps state; pixel density 2; `authored-extras`; a throw injected at a background call). Under the 1.75× camera in preview, in both the spotlight-reveal and always-show modes, the next frame drew `main clearRect … @ 3.5 0 0 3.5 -990 -171.6` where a clean frame draws `@ 2 0 0 2 0 0`. Under a 2× viewport zoom in edit mode, it drew `@ 4 0 0 4 -198 -521.4`. Both were still wrong two frames later. The independent review's probe threw at the reveal mask's `drawImage`: `destination-in` and the camera stayed in force, and the next frame drew the background at 3.063× and every fill, the route's composite included, under `destination-in`. | Put each of those `save`s in a `try/finally` that restores it, which leaves a clean frame's calls unchanged and unwinds every level on a throw; or have each host re-apply its canvas set-up after a frame that throws | One bad frame cannot corrupt the next, on either canvas | Low | S | DEF-36 (its state-keeping test recorder) | **P2** · Accepted 2026-09-25 |
| DEF-39 | SEG-012 | `RenderingService.js:1528-1529, 1601-1602, 1763-1764` (the path sets `lineCap`/`lineJoin` outside any save); `AreaHighlightRenderer.js:298-312` (`_applyBorderStyle` sets colour, width and dash, not cap or join) | **Proposed 2026-09-25 (DEF-36 review); accepted by the owner 2026-09-25, keeping the round caps and joins that steady frames draw today.** The vector layer's styles carry from one frame to the next. The route sets round caps and joins outside any `save`, and area-highlight borders, which the next frame draws first, inherit them, as does any other outline that sets neither. So the first frame after a resize, and the frame after a throw (DEF-36), draw those outlines with butt caps and miter joins, a dotted border's dots as squares, while later frames draw them round. At an instant where nothing sets them again, a paused editor keeps whichever look it had. The goldens cannot see it: they record calls, not the state a call draws with. | [C] probe on the DEF-36 branch (the test recorder keeps state). In edit mode, a freshly sized layer and a steady one at the same instant start from different styles: `strokeStyle`, `fillStyle`, `lineWidth` and round `lineCap`/`lineJoin` carried over, in `authored-extras`, `parm-aerial-walk` and `uon-open-day`. There was none in `nervous-system-flow`, nor in `authored-extras` in preview, and the review's probe found no draw-relevant difference outside edit mode. In `authored-extras` at 0.5%, the fresh look lasted three more frames. The independent review's probe compared only the state each draw call uses. In edit mode at 50%, the frame after a throw at any of the 844 vector calls stroked with butt caps and miter joins where a steady frame strokes round. | Set every style a draw relies on where it draws (`_applyBorderStyle` would set cap and join, as would any other outline that inherits them), or start each frame's layer from its own defaults. Either fixes which look is kept, so the owner chooses round or butt | A frame depends only on its instant, not on the frames before it | Low | S | DEF-36 (its state-keeping test recorder) | **P3** · Accepted 2026-09-25 |
| DEF-42 | SEG-002, SEG-005, SEG-012 | `wiringDom.js:362-389` (the inspector's beacon-style `change` resets the beacon, then emits `waypoint:style-changed`); `wiringBus.js:260-267` (that event only renders, refreshes the list, records undo and autosaves); `AnimationEngine.js:531-534` (the schedules are built only when timing is rebuilt) | **Proposed 2026-09-26 (DEF-08's review); awaiting the owner's decision — not yet scheduled.** Choosing a beacon style in the inspector does not rebuild the beacon schedules, so the editor goes on timing that waypoint's beacon by its old style, or not at all if it had none, until something else rebuilds timing, such as a mode switch. A video export rebuilds timing before its first frame (`exporting.js:156`), so exports are right; the editor's play and scrub are not, nor, with DEF-08, its pause. | [C] the review's probes: in Chromium, on `main` and the DEF-08 branch alike, grow chosen for Open day's first waypoint had no schedule 1.5 s later; in jsdom, on the DEF-08 branch, pop → grow on the second waypoint kept pop's schedule (early onset 1,017 ms, arrival 1,267 ms, hold end 3,267 ms) until `invalidateAnimationTiming()` gave grow's (634, 1,267 and 5,384 ms). | Rebuild timing when a beacon style changes, as the motion settings already do (`wiringControllers.js:941-991`), and test that the schedule follows the chosen style with no mode switch | The editor animates the beacon the inspector shows | Low | S | — | **P3** · Proposed 2026-09-26 |
| DEF-43 | SEG-014, SEG-019 | `AreaDrawingService.js:84-85` (only a project replacement or Clear All cancels a draw); `AreaDrawingService.js:92-95` (a new `area:draw-start` cancels the draw in progress); `AreaDrawingService.js:155-175` (completion writes to its target without checking that it is still in the model); `UIController.js:1418-1422` (Draw Area asks for a new draw each time it is activated) | **Proposed 2026-09-26 (DEF-17's review); awaiting the owner's decision — not yet scheduled.** A polygon draw outlives its target, and a stray key restarts it. Deleting the target waypoint (the list's ×) or undoing past it mid-draw leaves the tool drawing, and closing the polygon then writes it to a waypoint that is no longer in the project, so the polygon is lost without a word. And focus stays on Draw Area through the canvas clicks, so Space or Enter starts the draw again, discarding the vertices placed so far. | [C] the review's Chromium probes, on `main` and the DEF-17 branch alike: after Ctrl+Z mid-draw, closing the polygon left `targetInModel: false` and the live waypoint with `shape: none, points: 0`; after the target was deleted with ×, the draw went on and the polygon went to the deleted waypoint; Space after three vertices left none, still drawing. | Cancel the draw, and say so, when its target leaves the model (or refuse to commit to a target that is not in it), and stop Draw Area restarting a draw in progress; test both on the booted app | A drawn polygon is never lost silently | Low | S | DEF-17 | **P3** · Proposed 2026-09-26 |
| DEF-44 | SEG-009, SEG-005 | `CameraService.js:81-90` (with no waypoints, only the target centre is set); `CameraService.js:439-454` (`isZoomTransitioning` compares the smoothed centre with the target); `camera.js:246-251` (in Edit mode the camera is never advanced); `playback.js:252-254` (the render loop stays awake while the camera is transitioning) | **Proposed 2026-09-26 (DEF-33's and DEF-30's reviews); awaiting the owner's decision — not yet scheduled.** A paused editor that should be still keeps queuing frames, about 60 a second, against the hard rule that stable paused views queue no animation frame. The camera's smoothed centre starts at (0, 0) and its target at the canvas centre, and nothing moves the smoothed centre while there are no waypoints, or in Edit mode, so the camera never counts as settled and the loop never sleeps. It costs battery and responsiveness: with spotlight reveal the paused page ran frames of about 230 ms in headless Chromium, and the live region's 2 s clear fired 5–19 s late. | [C] the reviews' Chromium idle probes, on `main` and their branches alike: about 50–60 `animation:update` events a second, 10 s after a cold start with no waypoints and after restoring a preview-mode project (DEF-33's); about 60 frames a second in a settled, paused two-waypoint editor in Edit mode, with the smoothed centre at (0, 0) and the target near (350, 247) (DEF-30's). | Settle the smoothed centre with its target wherever the target moves and the camera does not advance (the no-waypoint branch, Edit mode), and add an idle test that no frame is queued once settled: after a cold start, after restoring a preview project, and paused in Edit mode | Paused views are idle, as the hard rule says | Low | S | — | **P2** · Proposed 2026-09-26 |
| DEF-45 | SEG-001, SEG-004 | `main.js:868-874` (`announce` replaces the live region's text, and never cancels an earlier message's 2 s clear); `persistence.js:633-646` (`reportAutosaveOmissions` announces what browser recovery leaves out), reached through `replaceImmediateRecovery` (`:648-664`) just before the restore (`:1235-1237`) and Open Project (`:1047-1049`) announce their own message | **Proposed 2026-09-26 (DEF-33's review); awaiting the owner's decision — not yet scheduled.** The live region keeps only the last message, so a warning announced just before another is never read. Restoring a legacy recovery point announces "Browser recovery excludes the background…" and then, in the same moment, "Previous session restored"; opening a project replaces the same warning with "Project loaded". So a screen-reader user is never told that their recovery point leaves out the background or custom images. By reading, an earlier message's 2 s clear can also blank a later message early. | [C] the review's Chromium probe (a MutationObserver on the live region), on `main` and the DEF-33 branch alike: legacy recovery with a background wrote the warning and then "Previous session restored" within one task. | Queue announcements so each is read in turn, or fold the warning into the restore and load messages, and let a new message cancel the previous one's clear; test that both texts reach the region | Recovery warnings reach screen-reader users | Low | S | — | **P3** · Proposed 2026-09-26 |
| DEF-46 | SEG-005, SEG-016, SEG-019 | `exporting.js:94-236` (`exportVideo` does its setup, then `VideoExporter.export` refuses a second export, `VideoExporter.js:424-429`, and the refused call's `finally` still restores the display); `exporting.js:69-77` (export mode is one boolean); `UIController.js:886-935` (Export MP4 awaits a codec probe before it asks for an export, and the buttons are disabled only once an export starts) | **Proposed 2026-09-26 (DEF-29's review); awaiting the owner's decision — not yet scheduled.** A second video export started while one is running spoils the first. The second `exportVideo()` repeats the setup (preview mode, timing, the reveal mask, the export canvas), is refused with "Export already in progress", and tells the author the export failed. Its `finally` then leaves export mode, restores the display canvas and re-enables the export buttons under the running export, so the rest of that video is drawn at display size without the export flag. The likely route is a double click on Export MP4: its codec probe is asynchronous, so both clicks can ask for an export before the first disables the buttons. | [C] the review's jsdom probe (D3), on the DEF-29 branch: after a second `exportVideo()` during the first, all 120 later frames were drawn unflagged and the canvas width fell from 1,920 to 1,173 px. The double click could not be tried: headless Chromium has no H.264 encoder. The failure message and the re-enabled buttons are by reading. | Refuse a second export before touching anything (a guard at the top of `exportVideo`, or disable the buttons before the probe), and test that a second request leaves the running export's frames unchanged | A double click cannot spoil a video | Low | S | — | **P2** · Proposed 2026-09-26 |
