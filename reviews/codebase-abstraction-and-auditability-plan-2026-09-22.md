...
| DEF-21 | SEG-023, SEG-025, SEG-021 | `keybindings.js:266-291`; `InteractionHandler.js:927-944`; `index.html:50` | Help labels `,` and `.` "Step" (they skip to start/end) and K "Pause" (it toggles), and the File menu advertises **⌘O**, which has no handler | [C] | Correct the text now; the structural fix is CON-15 | Help tells the truth | Low | S | — | **P1** |
...
| DEF-04 | SEG-006, SEG-014, SEG-004 | `NetworkEditService.js:315-331, 570`; `AreaDrawingService`; `crowds.js:568-575, 686-688`; load-side checks in `persistence`/models | Pointer and sidebar authoring bypass the limits that load enforces (MAX_WAYPOINTS, nodes, edges, control points, polygon points, crowd aggregates, the 600 s pause cap), so **saves won't reopen** | [C] grep: limits checked only in sceneOutline and at load | CON-11 budget helpers on all paths, plus a save-time self-check in TST-06 | Closes the class | Low | M | CON-11, TST-06 | **P1** |
...
| DEF-29 | SEG-012, SEG-016 | `BeaconRenderer.js:957-965, 1102-1108` | The author's OS reduced-motion setting suppresses beacons **in baked video exports** | [C] reading | Owner decides (§20 Q7b). Default: ignore reduced motion when exporting video; keep it for the live player. | Deterministic artefacts | Low | S | — | **P3** |
...
| CON-01 | SEG-019, SEG-003 | `UIController.updateWaypointEditor:2001-2145`; `editorPanel.js:333-485, 571-780` | 3 writers, divergent defaults, order dependence | [C] | Make editorPanel the single owner; delete UIController's writes | One source of truth | Medium | M | TST-04 | P2 |
