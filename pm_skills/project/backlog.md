...
- [x] Canvas affordances to match — shipped 2026-08-18 (see decision-log "Phase 4 second slice"): hover cursor + two-tone ring on waypoints and area handles; segment hit-testing on the leg the inspector names (hover glow under the path, click selects the owning waypoint and expands + flashes its Leg card — and a click near the path no longer drops an accidental major); midpoint "+" handle inserts a minor at exactly owner+1 on the path midpoint (one undo entry, selection follows); modifier gestures unchanged; hover is edit-mode-only and modifier cursors outrank it. New `utils/segmentHitTest.js` geometry util + 16 tests (259 total)
- [x] Layers strip + Crowd scope — shipped 2026-08-18 (see decision-log "Phase 4 third slice"): strip above the waypoint list (Route row + crowds: swatch/name/visibility eye/delete ×/double-click rename, "+ Add crowd" gated on a route existing); crowd selection switches the inspector to Crowd scope — Guide (Follow route; Custom network visible but disabled until network editing) · Dots (colour/size/wobble) · Release (count/window %) · Motion (speed/variance/lifecycle); chip "Editing · Crowd 1 · crowd" with green tint; one click adds a flowing sky-blue crowd (click 2 = play); scene undo/autosave reused, selection re-resolved by id after restores; new `src/app/crowds.js` mixin + 15 tests (274 total). Bonus fix: the waypoint:deselect handler had thrown since the mixin split — Escape deselection now actually works
- [x] Network edit mode (the one true tool mode, like polygon draw): same pen gestures on shared snapping/hit-testing services (click places node, drag curves edge); node/edge selection reuses the inspector pattern — Node card (pass-through/entry/exit), Edge card (direction toggle; weight displayed as computed junction traffic share, not a bare number) — shipped 2026-08-18 (see decision-log "Phase 4 fourth slice"): Guide card's Custom network option enabled — switching an empty-network crowd auto-enters the mode (forcing Edit mode via the canonical preview switch); pen chaining (click places a linked node, click a node continues from it, click an edge selects it and lifts the pen), drag moves nodes (Shift 15° snap vs first neighbour) / bends edges (control point inserted in chain order) / moves control handles; Shift-click deletes with undo toast; Esc ladder cancel-drag → lift pen → clear selection → exit; Node/Edge scopes join the one-inspector (chip "Editing · Node · entry" / "Editing · Edge · one-way", crowd green); edges render + hit-test through SwarmEngine's own geometry cache (now public `edgeGeometry`) so the drawn curve is the travelled curve; entry=triangle/exit=square/pass-through=circle glyphs, one-way midpoint arrows; network renders as passive display whenever a graph-guided crowd is selected (edit mode only); undo/restore re-binds by id (mode survives). New `src/services/NetworkEditService.js` + `src/app/network.js` + 28 tests (301 total)
- [x] Multi-select honoured by every card — shipped 2026-08-18 (see decision-log "Phase 4 fifth slice"): selection is now an app-level set with a primary (last clicked); the hidden "Select All Waypoints" list row, its warning modal, and the `waypoint:all-change` bulk pipeline are deleted; every card control writes to the whole selection through its normal single-change event (one path recalc, one debounced undo entry, one autosave per gesture) — leg/path props include minors, marker/beacon/label/pause/speed/camera stay majors-only, matching the disabled-control rule; Cmd/Ctrl+A now actually works (the old handler called a method that never existed) and selects minors too; canvas Cmd/Ctrl+click toggles membership; Delete removes the whole selection as one undo entry; arrow nudges move the group; chip counts the selection ("Editing · 4 waypoints (1 minor)"); selection rings on every member; undo snapshots carry `selectedWaypointIds` and restores re-bind by id. 25 new tests in tests/multiSelect.test.js (326 total)
