...
- Two handlers in `wiringControllers.js` place a waypoint next to one that may be off the image, then pull it onto the image whatever the background zoom: inserting before the first or after the last (`waypoint:insert-adjacent`, which continues the route by 8% and clamps to 0–1) and duplicating (`waypoint:duplicate`, which caps each coordinate at 1, so a copy of (1.5, −0.3) lands at (1, −0.28)); DEF-03's rule would let the new point follow when zoomed out. — (from: 2026-09-25 DEF-35)
