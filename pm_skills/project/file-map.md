...
- `tests/authorableLoadable.test.js` — TST-06 property tests: every shipped slider and select, at its bounds and over three seeds, saves a project that loads; the DEF-03/04 failures characterised with the reason the loader gives; and the DEF-31 regression (a crowd traced from the longest waypoint ids reloads)
